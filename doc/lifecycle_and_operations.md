...
- [kafka-shutdown.txt](./source/kafka-shutdown.txt) — graceful-shutdown patterns, ack modes, EKS-ready config and manifests.
- [PAUSE LISTENER.txt](./source/PAUSE%20LISTENER.txt) — `KafkaListenerEndpointRegistry`, `pause()` vs `stop()`, idle events.
- [app-control-pane.md](./source/app-control-pane.md) — keep a control plane to pause and investigate.
- [example-lifecycle.md](./source/example-lifecycle.md) — Spring lifecycle and listener lifecycle examples.
- [dist_scale_and_shutdown.md](./source/dist_scale_and_shutdown.md) — consumer processes/threads in EKS, partition-bounded parallelism, rebalance listeners and shutdown events.
- [k8s-config.txt](./source/k8s-config.txt) — Deployment manifest, probes, `preStop`, rolling-update strategy, CI/CD.
- [ecs-config.txt](./source/ecs-config.txt) — ECS task definition, `stopTimeout`, ALB health checks, EKS↔ECS mapping.
- [Signal.md](./source/Signal.md) — (Nuxt/TS async cancellation; named "Signal" but unrelated to OS signals).
- [rebalance.pdf](./source/rebalance.pdf) — consumer groups, rebalance triggers, `session.timeout.ms`/`heartbeat.interval.ms`/`max.poll.interval.ms`, eager vs incremental (cooperative) rebalance, static group membership, duplicate/storm risks.
- [EKS-ECS.docx](./source/EKS-ECS.docx) — ECS vs EKS comparison, why pods restart more, probe behaviour for Spring Boot.
- [EKSvsPODs.docx](./source/EKSvsPODs.docx) — pod lifecycle phases, signals, postStart/preStop hooks, the three probes.
- [sclaling.docx](./source/sclaling.docx) — KEDA event-driven autoscaling and Kafka consumer-lag scaling.
- [kafka-listener-lifecycle.zip](./source/kafka-listener-lifecycle.zip) — standalone sample of registry-driven listener pause/resume/stop (reference only).
