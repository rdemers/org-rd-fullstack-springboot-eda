...
#
# 7. To clear cache and local registry to do a full test of the construction of your image:
...
# Build requires Node.js and Java/JDK.
    ...
    RUN echo "deb [signed-by=/usr/share/keyrings/nodesource.gpg] https://deb.nodesource.com/node_22.x nodistro main" | tee /etc/apt/sources.list.d/nodesource.list
    RUN apt update -y
...
#
# Prepare the SpringBoot application layers.
