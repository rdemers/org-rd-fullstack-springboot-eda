...
import org.rd.fullstack.springbooteda.srv.RequestProcessorSrv;
...
@DisplayName("Processor demo and tests.")
public class T1200_DBProcessor_UT_Tests {
    private static final Logger logger = 
        LoggerFactory.getLogger(T1200_DBProcessor_UT_Tests.class);

    @Autowired
    private DataGeneratorSrv dataGenerator;

    @Autowired
    RequestProcessorSrv requestProcessor;
    ...
    public T1200_DBProcessor_UT_Tests() {
        super();
    }

    @Test
    @Order(1)
    public void processorDemoTest() {
        ...
        lstRequest.forEach(request -> {
                try {
                    String jSonRequest = JsonMapper.writeToJson(request);
                    requestProcessor.process(jSonRequest);
                } catch (Exception ex) {
