...
public record TopicConfig (
    ...
    public TopicConfig {
        ...
        Objects.requireNonNull(dltName, "DLT name must not be null (use empty string for no DLT).");
        if (name.isBlank())
    ...
    public TopicConfig(String name, String dltName, int partitions, short replicas,
        ...
        this(name, dltName, partitions, replicas, KafkaConstants.CST_RETRY_ATTEMPTS,
                                                  KafkaConstants.CST_RETRY_INTERVAL, keySerializer, valueSerializer,
    ...
    public static boolean isReservedTopicName(String topicName) {
        return KafkaConstants.CST_RESERVED_TOPIC_NAMES.stream()
