...
<template>
  <v-container>
    ...
    <v-row class="mt-4 px-3 gap-2" align="start" no-gutters>
      ...
      <v-spacer/>
      <v-text-field v-model.number="refillAmount" type="number" :min="1" :max="1000"
        :label="t('person.balance')" density="compact" variant="outlined"
        hide-details="auto" :rules="refillRules" style="max-width: 150px"/>
      <v-btn color="success" prepend-icon="mdi-cash-plus" :disabled="!isRefillValid" @click="refill">
        {{ t('common.button.fill') }}
      </v-btn>
    </v-row>
...
<script setup lang="ts">
    ...
    const REFILL_MIN = 1;
    const REFILL_MAX = 1000;
    
    const persons = ref<Person[]>([]);
    const loading = ref(false);
    const deleteDialog = ref(false);
    const idToDelete = ref<number | null>(null);
    const refillAmount = ref<number>(100);

    const refillRules = computed(() => [
      (v: number | null) =>
        (v !== null && Number.isInteger(v) && v >= REFILL_MIN && v <= REFILL_MAX) ||
        t('common.rule.between', { field: t('person.balance'), min: REFILL_MIN, max: REFILL_MAX })
    ]);

    const isRefillValid = computed(() =>
      Number.isInteger(refillAmount.value) &&
      refillAmount.value >= REFILL_MIN &&
      refillAmount.value <= REFILL_MAX
    );
    ...
    async function refill() {
        if (!isRefillValid.value)
            return;

        loading.value = true;

        try {
            const response = await PersonService.refill(refillAmount.value);
            persons.value = (!response || !response.data) ? [] : response.data;
            notify(t("common.message.refill-success"));
        } catch (err) {
            console.error(err);
            const msg = err instanceof Error ? err.message : String(err);
            notify(t("person.refill-failed", { message: msg }), "error");
        } finally {
            loading.value = false;
        }
    }
