    ...
    <v-card class="mx-auto mt-5" max-width="900" elevation="10" border>
      ...
      <v-card-subtitle class="py-3 px-4 bg-grey-lighten-4">
        <v-row density="compact" justify="space-between">
          <span>
            <span class="mr-2 text-caption font-weight-bold">{{ t('kafka.cluster-id') }}</span>
            <code class="font-weight-bold">{{kafkaDashboard?.clusterId}}</code>
          </span>
          <span>
            <v-badge dot :color="errorMessage ? 'red' : 'success'" inline class="mr-1 custom-dot"/>
