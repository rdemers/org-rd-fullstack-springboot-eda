...
class RequestService {
    ...
    public getAllView(): Promise<AxiosResponse<RequestView[]>> {
        return apiClient().get<RequestView[]>(`${this.endpoint}/view`);
    }

    public getCount(): Promise<AxiosResponse<RequestCount>> {
        return apiClient().get<RequestCount>(`${this.endpoint}/count`);
    }

    public resetAll(result: number): Promise<AxiosResponse<RequestView[]>> {
        return apiClient().post<RequestView[]>(`${this.endpoint}/reset/${result}`);
    }
}
