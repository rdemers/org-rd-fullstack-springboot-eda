      ...
      <v-row>
        ...
        <v-col v-else cols="12">
          <v-autocomplete
            ...
            :label="t('inventory.productdescr')"
            :item-title="p => `${p.code} - ${p.description}`"
...
<script setup lang="ts">
    ...
    watch(() => props.inventoryView, (newVal) => {
        localInventoryView.value = { ...newVal };
    }, { immediate: true });
    ...
    onMounted(async () => {
        ...
        try {
            ...
            products.value = response.data.map((p: Product) => ({...p}));
        } catch (err) {
    ...
    watch(selectedProduct, (newProd) => {
      if (newProd) {
          localInventoryView.value.productId = newProd.productId;
          localInventoryView.value.productCode = newProd.code;
          localInventoryView.value.productDescription = newProd.description;
      } else {
          localInventoryView.value.productId = null;
          localInventoryView.value.productCode = "";
          localInventoryView.value.productDescription = "";
      }
    });
